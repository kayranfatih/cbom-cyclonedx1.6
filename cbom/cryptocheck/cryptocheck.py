...
def validate_cbom(cbom, rules_file=None, *, enrich_cbom=True):
    ...
    for rule in rules:
        ...
        algorithm_components = (
            ...
            and getattr(getattr(c, 'crypto_properties', None), 'asset_type', None) == CryptoAssetType.ALGORITHM
        )

        for algorithm_component in algorithm_components:
            variant = _build_variant(algorithm_component)

            is_match = map(
                lambda pattern: _VALIDATORS[pattern[1]](pattern[2], variant.get(pattern[0])),
                rule['patterns']
            )
            if all(is_match):
...
def _build_variant(component):
    properties = getattr(getattr(component, 'crypto_properties', None), 'algorithm_properties', None)

    algo_name = (component.name or '').upper()
    if '-' in algo_name:
        algo_name = algo_name.split('-', 1)[0]

    if properties:
        keylen = properties.parameter_set_identifier
        if isinstance(keylen, str) and keylen.isdigit():
            keylen_value = int(keylen)
        else:
            keylen_value = keylen

        mode = properties.mode
        mode_value = _enum_to_string(mode)

        padding = properties.padding
        padding_value = _enum_to_string(padding)
    else:
        keylen_value = None
        mode_value = ''
        padding_value = ''
